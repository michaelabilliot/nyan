...
export const UPGRADES_DATA = [
	...
	{ id: 'quantum_mouse', name: 'Quantum Mouse', description: 'A mouse that exists in multiple places at once, boosting your clicks.', type: 'click', baseCost: 2.2e7, power: 9000, costIncrease: 1.035 },
    { id: 'rainbow_factory', name: 'Rainbow Factory', description: 'Mass produces rainbows, a key ingredient in Nyan Coins.', type: 'cps', baseCost: 8.1e7, power: 100000, costIncrease: 1.036 },
    { id: 'meme_archive', name: 'Meme Archive', description: 'Unlocks ancient memes, giving you powerful clicks.', type: 'click', baseCost: 3e8, power: 25000, costIncrease: 1.033 },
    { id: 'poptart_catapult', name: 'Poptart Catapult', description: 'Launches Poptarts into the cosmos, generating coins.', type: 'cps', baseCost: 9e8, power: 200000, costIncrease: 1.036 },
    { id: 'nyan_choir', name: 'Nyan Choir', description: 'A choir of Nyan Cats singing the song of their people, boosting CPS.', type: 'click', baseCost: 1.8e9, power: 100000, costIncrease: 1.036 },
    { id: 'internet_core', name: 'Internet Core', description: 'Harness the power of the internet itself for massive click power.', type: 'cps', baseCost: 3e9, power: 15000000, costIncrease: 1.041 },
...
export const ACHIEVEMENTS_DATA = {
    Clicking: {
        'click_1': { name: 'The Journey Begins', description: 'Make your first click.', condition: (gs) => gs.totalClicks >= 1 },
        'click_100': { name: 'Getting Warmed Up', description: 'Click 100 times.', condition: (gs) => gs.totalClicks >= 100 },
        'click_1k': { name: 'Clicker Trainee', description: 'Click 1,000 times.', condition: (gs) => gs.totalClicks >= 1000 },
        'click_10k': { name: 'Carpal Tunnel Incoming', description: 'Click 10,000 times.', condition: (gs) => gs.totalClicks >= 10000 },
        'click_100k': { name: 'Master Clicker', description: 'Click 100,000 times.', condition: (gs) => gs.totalClicks >= 100000 },
        'click_1m': { name: 'Mouse Destroyer', description: 'Click 1,000,000 times.', condition: (gs) => gs.totalClicks >= 1000000 },
        'click_10m': { name: 'Finger of the Gods', description: 'Click 10,000,000 times.', condition: (gs) => gs.totalClicks >= 10000000 },
    },
    Production: {
        'cps_1k': { name: 'Automatic Start', description: 'Reach 1,000 Nyan Coins per second.', condition: (gs) => calculateTotalCPS(gs) >= 1000 },
        'cps_100k': { name: 'Rainbow Engine', description: 'Reach 100,000 Nyan Coins per second.', condition: (gs) => calculateTotalCPS(gs) >= 100000 },
        'cps_1m': { name: 'Industrial Revolution', description: 'Reach 1 Million Nyan Coins per second.', condition: (gs) => calculateTotalCPS(gs) >= 1e6 },
        'cps_100m': { name: 'Planetary Production', description: 'Reach 100 Million Nyan Coins per second.', condition: (gs) => calculateTotalCPS(gs) >= 1e8 },
        'cps_1b': { name: 'Galactic Factory', description: 'Reach 1 Billion Nyan Coins per second.', condition: (gs) => calculateTotalCPS(gs) >= 1e9 },
        'cps_1t': { name: 'Universal Output', description: 'Reach 1 Trillion Nyan Coins per second.', condition: (gs) => calculateTotalCPS(gs) >= 1e12 },
    },
    Acquisition: {
        'coins_1k': { name: 'Coin Collector', description: 'Possess 1,000 Nyan Coins.', condition: (gs) => gs.coins >= 1000 },
        'coins_1m': { name: 'Nyan Millionaire', description: 'Possess 1 Million Nyan Coins.', condition: (gs) => gs.coins >= 1e6 },
        'coins_1b': { name: 'Nyan Billionaire', description: 'Possess 1 Billion Nyan Coins.', condition: (gs) => gs.coins >= 1e9 },
        'coins_1t': { name: 'Nyan Trillionaire', description: 'Possess 1 Trillion Nyan Coins.', condition: (gs) => gs.coins >= 1e12 },
        'coins_1q': { name: 'Cosmic Tycoon', description: 'Possess 1 Quadrillion Nyan Coins.', condition: (gs) => gs.coins >= 1e15 },
        'coins_1qt': { name: 'Nyan God', description: 'Possess 1 Quintillion Nyan Coins.', condition: (gs) => gs.coins >= 1e18 },
    },
    Construction: {
        'upgrade_total_1': { name: 'First Step', description: 'Buy your first upgrade.', condition: (gs) => Object.keys(gs.upgrades).length > 0 },
        'upgrade_total_100': { name: 'Upgrade Enthusiast', description: 'Buy 100 total upgrades.', condition: (gs) => Object.values(gs.upgrades).reduce((sum, u) => sum + u.owned, 0) >= 100 },
        'upgrade_total_500': { name: 'Hoarder', description: 'Buy 500 total upgrades.', condition: (gs) => Object.values(gs.upgrades).reduce((sum, u) => sum + u.owned, 0) >= 500 },
        'upgrade_total_1k': { name: 'Architect', description: 'Buy 1,000 total upgrades.', condition: (gs) => Object.values(gs.upgrades).reduce((sum, u) => sum + u.owned, 0) >= 1000 },
        'upgrade_total_2.5k': { name: 'Empire Builder', description: 'Buy 2,500 total upgrades.', condition: (gs) => Object.values(gs.upgrades).reduce((sum, u) => sum + u.owned, 0) >= 2500 },
        'sprinkles_250': { name: 'Sweet Tooth', description: 'Own 250 Sprinkles.', condition: (gs) => gs.upgrades['sprinkles']?.owned >= 250 },
        'lovebots_250': { name: 'Full of Love', description: 'Own 250 Lovebots.', condition: (gs) => gs.upgrades['lovebots']?.owned >= 250 },
        'rainbow_mice_250': { name: 'Pied Piper', description: 'Own 250 Rainbow Mice.', condition: (gs) => gs.upgrades['rainbow_mice']?.owned >= 250 },
        'auto_petter_250': { name: 'Hands Off', description: 'Own 250 Auto-Petters.', condition: (gs) => gs.upgrades['auto_petter']?.owned >= 250 },
        'catnip_farm_250': { name: 'Green Thumb', description: 'Own 250 Catnip Farms.', condition: (gs) => gs.upgrades['catnip_farm']?.owned >= 250 },
        'keyboard_cat_250': { name: 'Play Him Off', description: 'Own 250 Keyboard Cats.', condition: (gs) => gs.upgrades['keyboard_cat']?.owned >= 250 },
        'laser_pointer_250': { name: 'Red Dot Obsession', description: 'Own 250 Laser Pointers.', condition: (gs) => gs.upgrades['laser_pointer']?.owned >= 250 },
        'yarn_ball_250': { name: 'All Tangled Up', description: 'Own 250 Yarn Balls.', condition: (gs) => gs.upgrades['yarn_ball']?.owned >= 250 },
        'quantum_mouse_250': { name: 'Schrödinger\'s Pet', description: 'Own 250 Quantum Mice.', condition: (gs) => gs.upgrades['quantum_mouse']?.owned >= 250 },
        'rainbow_factory_250': { name: 'Double Rainbow', description: 'Own 250 Rainbow Factories.', condition: (gs) => gs.upgrades['rainbow_factory']?.owned >= 250 },
        'meme_archive_250': { name: 'Ancient Knowledge', description: 'Own 250 Meme Archives.', condition: (gs) => gs.upgrades['meme_archive']?.owned >= 250 },
        'poptart_catapult_250': { name: 'Breakfast Artillery', description: 'Own 250 Poptart Catapults.', condition: (gs) => gs.upgrades['poptart_catapult']?.owned >= 250 },
        'nyan_choir_250': { name: 'Harmonic Convergence', description: 'Own 250 Nyan Choirs.', condition: (gs) => gs.upgrades['nyan_choir']?.owned >= 250 },
        'internet_core_250': { name: 'Series of Tubes', description: 'Own 250 Internet Cores.', condition: (gs) => gs.upgrades['internet_core']?.owned >= 250 },
        'singularity_250': { name: 'Event Horizon', description: 'Own 250 Singularities.', condition: (gs) => gs.upgrades['singularity']?.owned >= 250 },
    },
    Prestige: {
        'rebirth_1': { name: 'Again!', description: 'Perform your first rebirth.', condition: (gs) => gs.rebirths >= 1, reward: 100000 },
        'rebirth_5': { name: 'Time Loop', description: 'Rebirth 5 times.', condition: (gs) => gs.rebirths >= 5 },
        'rebirth_10': { name: 'Loop Master', description: 'Rebirth 10 times.', condition: (gs) => gs.rebirths >= 10 },
        'rebirth_25': { name: 'Déjà Vu', description: 'Rebirth 25 times.', condition: (gs) => gs.rebirths >= 25 },
        'rebirth_50': { name: 'Timeless Cat', description: 'Rebirth 50 times.', condition: (gs) => gs.rebirths >= 50 },
        'rebirth_100': { name: 'Ouroboros', description: 'Rebirth 100 times.', condition: (gs) => gs.rebirths >= 100 },
    ...
    'Nyan Tree': {
        ...
        'tree_nodes_10': { name: 'Branching Out', description: 'Purchase 10 total node levels in the Nyan Tree.', condition: (gs) => Object.values(gs.nyanTreeUpgrades).reduce((a, b) => a + b, 0) >= 10 },
    ...
    Mastery: {
        ...
        'achieve_25': { name: 'Dedicated', description: 'Unlock 25 achievements.', condition: (gs) => gs.unlockedAchievements.length >= 25 },
        'achieve_50': { name: 'Completionist', description: 'Unlock 50 achievements.', condition: (gs) => gs.unlockedAchievements.length >= 50 },
